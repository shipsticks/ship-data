...
drop table if exists adhoc.user_ltv_3;
create table if not exists adhoc.user_ltv_3 as 
with tmp as (
select
  brand
  , user_id
...
drop table if exists adhoc.traveler_ltv_4;
create table if not exists adhoc.traveler_ltv_4 as 
with tmp as (
select
  brand
  ...
  , sum(price_cents) / 100 as total_revenue
from `dp_bi.finsum` as f
join `mongo_land.travelers` as t
  on f.traveler_id = t._id
where 
  user_is_pro = true
  -- and traveler_email is not null
group by all
